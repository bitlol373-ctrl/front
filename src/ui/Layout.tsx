...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const loadMe = async () => {
    ...
    setLoadingMe(true)
    try {
      const res = await fetch(`${API}/auth/me`, { headers: authHeaders(tok) })
      if (!res.ok) {
      ...
      // если бэк недоступен/корс/ошибка сети
      setMe(null)
    } finally {
      setLoadingMe(false)
    }
...
function TopLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        ['rounded-xl px-3 py-2 transition', isActive ? 'bg-white/10 text-white' : 'hover:bg-white/5 hover:text-white'].join(
          ' ',
        )
      }
    >
